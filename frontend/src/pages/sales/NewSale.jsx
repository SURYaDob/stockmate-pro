...
import {
  ArrowLeft, Search, Plus, Minus, X, Save, ShoppingCart,
  Loader2, AlertTriangle, Check, Trash2, User, Package,
  Percent, DollarSign
} from 'lucide-react';
...
const NewSale = () => {
  ...
  // Quick product search for direct add
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">New Sale Invoice</h1>
          <p className="text-sm text-slate-400 mt-1">Create a new invoice with line items</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {selectedCustomer ? (
                <div className="flex items-center justify-between p-3 rounded-lg bg-slate-50 dark:bg-slate-700/50">
                  <div>
                    ...
                    <p className="text-sm text-slate-400">{selectedCustomer.phone}{selectedCustomer.gstNumber ? ` \u2022 GST: ${selectedCustomer.gstNumber}` : ''}</p>
                  </div>
                  <button
                    type="button"
                    ...
                    <div className="absolute top-full left-0 right-0 mt-1 z-20 bg-white dark:bg-slate-800 rounded-lg shadow-lg border border-slate-200 dark:border-slate-700 max-h-48 overflow-y-auto animate-scale-in">
                      {customers.map((c) => (
                        ...
                        >
                          ...
                          <div>
                            <p className="font-medium text-slate-700 dark:text-slate-300">{c.name}</p>
                            <p className="text-xs text-slate-400">{c.phone}{c.gstNumber ? ` \u2022 ${c.gstNumber}` : ''}</p>
                          </div>
                        </button>
                      ))}
                    </div>
                  ...
                  <table className="w-full min-w-[600px]">
                    <thead>
                      <tr className="border-b border-slate-200 dark:border-slate-700">
                        ...
                        <th className="text-right text-xs font-medium text-slate-400 uppercase tracking-wider pb-2">Disc</th>
                          ...
                          <tr key={li.itemId} className="border-b border-slate-100 dark:border-slate-700">
                            ...
                            <td className="py-2 text-right">
                              <input
                                ...
                                min="0"
                                value={li.discount}
                                onChange={(e) => updateLineItem(li.itemId, 'discount', parseInt(e.target.value) || 0)}
                                className="w-20 text-right input-sm font-mono"
                                placeholder="0"
                              />
                            </td>
                            <td className="py-2 text-right">
        ...
        <div className="space-y-6">
          ...
          <div className="card">
            <div className="card-header">
              <h2 className="text-base font-semibold text-slate-800 dark:text-slate-200">Invoice Summary</h2>
            </div>
          ...
          >
            {saving ? (
              <><Loader2 size={20} className="animate-spin" /> Creating Invoice...</>
            ) : (
